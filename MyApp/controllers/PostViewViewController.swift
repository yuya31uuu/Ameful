...
class PostViewViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate, UITextViewDelegate,UITextFieldDelegate {
    ...
    func confirmContext() {
        if postTextView.text.count > 0 && directionTextField.text!.count > 0 {
    ...
    @IBAction func sharePhoto() {
        ...
        file.saveInBackground({ (error) in
            if error != nil {
                ...
                postObject?.setObject(self.directionTextField.text, forKey: "direction")
                postObject?.setObject(self.postTextView.text!, forKey: "text")
                postObject?.setObject(NCMBUser.current(), forKey: "user")
                postObject?.setObject(self.directionTextField.text, forKey: "direction")
                let url = "https://mbaas.api.nifcloud.com/2013-09-01/applications/QYEIUecMOiGT2A6H/publicFiles/" + file.name
                ...
                postObject?.saveInBackground({ (error) in
                    if error != nil {
                        ...
                        self.directionTextField.text = nil
